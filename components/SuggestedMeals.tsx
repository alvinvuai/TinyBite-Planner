...
export function SuggestedMeals({ meals, selectedId, onSelect }: SuggestedMealsProps) {
  ...
  return (
    <section className="soft-card space-y-3 p-4">
      <div>
        <p className="text-sm font-black text-[#633d55]">Suggested meal ideas</p>
        <p className="text-xs font-semibold leading-5 text-[#8a6679]">Tap one only if you want recipe details. Quantities stay editable below.</p>
      </div>
          ...
          return (
            <button
              ...
              className={`pressable rounded-[8px] border p-3 text-left ${
                active
                  ? "border-[#ff83b7] bg-[#ffe0ec] shadow-[0_14px_25px_rgba(255,131,183,0.22)]"
                  : "border-white/80 bg-white/68 shadow-sm"
              }`}
            >
