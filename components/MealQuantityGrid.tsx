  ...
  if (!items.length) {
    return (
      <section className="soft-card p-4 text-sm font-bold leading-6 text-[#765066]">
        Select foods on the left and Dưa Béo will build suggested portions here.
      </section>
    );
  }
    ...
    <section className="space-y-4">
      <div className="soft-card space-y-4 p-4">
        ...
        <div className="overflow-hidden rounded-[8px] border border-white/80 bg-white/62">
          <div className="grid grid-cols-[0.9fr_0.72fr_1.1fr_0.45fr] gap-2 bg-[#fff8fb] px-3 py-2 text-[9px] font-black uppercase tracking-[0.06em] text-[#9c456c] sm:text-xs">
              ...
              return (
                <div key={item.id} className="grid grid-cols-[0.9fr_0.72fr_1.1fr_0.45fr] items-start gap-2 px-3 py-3">
                  <div className="min-w-0">
                    <p className="text-sm font-black leading-5" style={{ color: style.text }}>
                      {item.name}
                    </p>
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-black leading-5 text-[#633d55]">
                      {formatAmount(item.suggestedAmount)} <span className="text-[10px] text-[#9a7487]">{suggestedUnit.label}</span>
                    </p>
                    <UnitMenu
                    ...
                    />
                    <p className="mt-1 text-[10px] font-semibold leading-4 text-[#9a7487]">{item.suggestedCalories} kcal target</p>
                  </div>
                  <div className="grid min-w-0 gap-2">
                    ...
                    />
                    <AmountSlider item={item} unit={unit} onChange={(amount) => updateItem(item.id, updateMealBuilderItem(item, amount))} />
                  </div>
...
function AmountSlider({ item, unit, onChange }: { item: MealBuilderItem; unit: UnitOption; onChange: (amount: number) => void }) {
  const max = Math.max(unit.min + unit.step, item.suggestedAmount * 2.2, item.amount * 1.2);
  const pct = Math.min(100, Math.max(0, ((item.amount - unit.min) / Math.max(unit.step, max - unit.min)) * 100));
  const pink = `color-mix(in srgb, #ffb9d5 ${Math.max(35, pct)}%, #7b3757)`;

  return (
    <input
      aria-label={`Quick adjust ${item.name}`}
      type="range"
      min={unit.min}
      ...
      style={{
        background: `linear-gradient(90deg, ${pink} 0 ${pct}%, #f7ddea ${pct}% 100%)`,
      }}
    />
  );
}
